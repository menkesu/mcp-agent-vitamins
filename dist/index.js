...
async function fetchBrief(token, date) {
    ...
    if (response.status === 403) {
        ...
        throw new Error(`Access denied: ${body.error}\n\nTo get a token:\n1. Visit https://agentvitamins.com\n2. Subscribe ($7/mo, 10-day free trial)\n3. You'll receive your API token via email`);
    }
    if (response.status === 429) {
        throw new Error("Rate limited (20 requests/min). Wait a moment and try again.");
    }
    if (!response.ok) {
...
function formatFullBrief(data) {
    const lines = [];
    lines.push(`# Agent Vitamins Daily Brief`);
    lines.push(`Generated: ${data.meta.generated_at}`);
    lines.push(`Sources scanned: ${data.meta.sources_scanned} | Items: ${data.meta.items_passed} passed, ${data.meta.items_failed} filtered out`);
    lines.push("");
    const selfImprovement = data.items.filter((item) => item.self_improvement_action && item.self_improvement_action.trim() !== "");
...
function formatPreview(data) {
    ...
    lines.push("");
    lines.push("---");
    lines.push("Get the full brief with actionable insights at https://agentvitamins.com — $7/mo, 10-day free trial");
    return lines.join("\n");
}
...
server.tool("get_daily_brief", "Fetch today's Agent Vitamins daily brief — AI news and self-improvement actions curated from 1000+ sources. Requires an API token from agentvitamins.com.", {
    token: z
        .string()
...
server.tool("get_brief_preview", "Get a free preview of today's Agent Vitamins brief — titles and one-line summaries. No token needed.", {
    date: z
        .string()
        .regex(/^\d{4}-\d{2}-\d{2}$/, "Date must be in YYYY-MM-DD format")
        .optional()
